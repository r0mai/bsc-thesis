...
\begin{lstlisting}[caption={CMake Invocation without the Plugin}]
mkdir build
cd build
cmake -G "Ninja" ..
\end{lstlisting}
do
\begin{lstlisting}[caption={CMake Invocation with the Plugin}]
mkdir build
cd build
cmake -G "Ninja" \
...
\lstinputlisting[language=c++, numbers=left, caption={\lstinline|example.cpp|}]
		{example.cpp}
